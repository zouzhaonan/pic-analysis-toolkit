...
`$wd/enrich/<genome>/` 配下に `GSEA/<method>/*.csv` と `ORA/<method>/*.csv` が作成されれば正常です(図は `pic report` が CSV から生成します)。
...
`Warning` が出ることがありますが、`$wd/deseq2/plot_expression_test.pdf` が正常にできていれば問題ありません。
